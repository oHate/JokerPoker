...
public class GamePane extends StackPane {
    ...
    public List<PlayingCardNode> addCards(List<PlayingCard> cards) {
        List<PlayingCardNode> views = new ArrayList<>();

        for (PlayingCard card : cards) {
            views.add(new PlayingCardNode(card));
        }
    ...
    public void sortCards(SortMode mode) {
        FXCollections.sort(this.cardArea.getChildren(), (a, b) -> {
            if (!(a instanceof PlayingCardNode viewA) || (!(b instanceof PlayingCardNode viewB))) return 0;
    ...
    public void discard() {
        this.cardArea.getChildren().removeIf(node -> {
            if (!(node instanceof PlayingCardNode playingCardView)) return false;
